# cause-specific version of ExC. 
# ExCl for ExCl
# ExCAl for ExCAl
# ExCl_pdi for ExCl*
# ExCAl_pdi for ExCAl*
ExCl=function(mydata,predicttime,cause){
...
ExCAl=function(mydata,predicttime,cause){
  colnames(mydata)=c("obstime","causesta", "cif1", "cif2", "s")
  mydata$trueclass=ifelse((mydata$obstime<=predicttime & mydata$causesta==1), 1, ifelse((mydata$obstime<=predicttime & mydata$causesta==2),2,ifelse((mydata$obstime>predicttime),3,NA))) 
  mydata$predsta=apply(mydata[,3:5],1,which.max)
  mydata$correct=as.numeric(mydata$trueclass==mydata$predsta)
  ...
  if(cause==1){
    class1comp=cpmdataobs$correct[obsclass1]
    class2comp=as.numeric(cpmdataobs$predsta!=1)[obsclass2]
    class3comp=as.numeric(cpmdataobs$predsta!=1)[obsclass3]
    part1=sum((class1comp%*%t(class2comp))/(probcen.event1%*%t(probcen.event2)))
    part2=sum((class1comp%*%t(class3comp))/(probcen.event1%*%t(probcen.event3)))
    Num.IPCW=part1+part2
    Denom.IPCW=sum((event1n%*%t(event2n))/(probcen.event1%*%t(probcen.event2)))+sum((event1n%*%t(event3n))/(probcen.event1%*%t(probcen.event3)))
    return(Num.IPCW/Denom.IPCW)
  }
  else if(cause==2){
    class1comp=as.numeric(cpmdataobs$predsta!=2)[obsclass1]
    class2comp=cpmdataobs$correct[obsclass2]
    class3comp=as.numeric(cpmdataobs$predsta!=2)[obsclass3]
    part1=sum((class2comp%*%t(class1comp))/(probcen.event2%*%t(probcen.event1)))
    part2=sum((class2comp%*%t(class3comp))/(probcen.event2%*%t(probcen.event3)))
    Num.IPCW=part1+part2
    Denom.IPCW=sum((event2n%*%t(event1n))/(probcen.event2%*%t(probcen.event1)))+sum((event2n%*%t(event3n))/(probcen.event2%*%t(probcen.event3)))
    return(Num.IPCW/Denom.IPCW)
  }
  else{
    return(NA)
...
ExCl_pdi=function(mydata,predicttime,cause){
  colnames(mydata)=c("obstime","causesta", "cif1", "cif2", "s")
  mydata$trueclass=ifelse((mydata$obstime<=predicttime & mydata$causesta==1), 1, ifelse((mydata$obstime<=predicttime & mydata$causesta==2),2,ifelse((mydata$obstime>predicttime),3,NA))) 
  mydata$predsta=apply(mydata[,3:5],1,which.max)
  mydata$correct=as.numeric(mydata$trueclass==mydata$predsta)
  ...
  if(cause==1){
    ...
    for(i in 1:nclass1){
      for(k in 1:nclass3){
        ...
        ck=(Sk>=Si)/(1+(Sk==Si)) # have testing that when ck=1, this is equal to AUC.
    ...
    for(j in 1:nclass2){
      for(k in 1:nclass3){
        ...
        ck=(Sk>=Sj)/(1+(Sk==Sj)) # have testing that when ck=1, this is equal to AUC.
...
ExCAl_pdi=function(mydata,predicttime,cause){
  colnames(mydata)=c("obstime","causesta", "cif1", "cif2", "s")
  mydata$trueclass=ifelse((mydata$obstime<=predicttime & mydata$causesta==1), 1, ifelse((mydata$obstime<=predicttime & mydata$causesta==2),2,ifelse((mydata$obstime>predicttime),3,NA))) 
  mydata$predsta=apply(mydata[,3:5],1,which.max)
  mydata$correct=as.numeric(mydata$trueclass==mydata$predsta)
  ...
  if(cause==1){
    ...
    for(i in 1:nclass1){
      for(j in 1:nclass2){
        ...
        if(F1i>=F1j){
          Num.IPCW=Num.IPCW+1/(probcen.event1[i]*probcen.event2[j])
        }
        else{
          next
        }
    ...
    for(i in 1:nclass2){
      for(k in 1:nclass3){
        F2i=class2comp$cif2[i]
        F2k=class3comp$cif2[k]
        if(F2i>=F2k){
          Num.IPCW=Num.IPCW+1/(probcen.event2[i]*probcen.event3[k])
        }
        else{
          next
        }
    ...
    for(i in 1:nclass2){
      for(j in 1:nclass1){
        F2i=class2comp$cif2[i]
        F2j=class1comp$cif2[j]
        if(F2i>=F2j){
          Num.IPCW=Num.IPCW+1/(probcen.event2[i]*probcen.event1[j])
        }
        else{
          next
        }
